    ...
    private func captureStartupFailureReason() {
        ...
        func resolve(from attempt: Int) {
            ...
            if attempt >= 4 {
                ...
                if extensionSilent {
                    lastError = String(localized: "Packet Tunnel did not start (no extension logs). Large config is now passed via VPN profile only — reinstall 0.1.0 (38), or check NE signing in ESign.")
